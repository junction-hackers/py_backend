...
import uuid
from matching import detectMatchingSearchers
...
def process_event(bucket, key):
    ...
    with s3utils.s3_download(bucket, key) as path:
        ...
        for index, result in enumerate(detect_facial_feature(path)):
            ...
            if initial:
                update_victim_info(_id, album_id, album_key, age, gender)
                initial = False

            directory = os.path.dirname(path)
            cropped_path = os.path.join(directory, f"{index}.jpg")
            crop_image_bounds(path, face, cropped_path)
            cropped_key =  f"cropped/{_id}/{index}.jpg"
            s3utils.s3_upload(bucket, cropped_key, cropped_path)

            add_face(album_id, album_key, bucket, album_key, index)
        detectMatchingSearchers(_id)
...
if __name__== "__main__":
    record = {'Records': [{'eventVersion': '2.1', 'eventSource': 'aws:s3', 'awsRegion': 'ap-northeast-1', 'eventTime': '2019-02-16T02:06:01.561Z', 'eventName': 'ObjectCreated:Put', 'userIdentity': {'principalId': 'AWS:AIDAIX2WJXJZSZE2473VO'}, 'requestParameters': {'sourceIPAddress': '180.3.139.234'}, 'responseElements': {'x-amz-request-id': '43747DDA3644C8CE', 'x-amz-id-2': 'o9DOPie1rVtsxVRN3/+HkKBPsJXc7OyKs/O9l6E9VNI7owc4sE9wFQRzxaZytdiQF3dElRhadT4='}, 's3': {'s3SchemaVersion': '1.0', 'configurationId': 'f27b5f7c-e4bc-414e-83d3-73d675e20377', 'bucket': {'name': 'hacktoncloudapi-tokyojunctionvictimbucket-1dt0qaq4h9akn', 'ownerIdentity': {'principalId': 'A2MJ4PT31WU8O7'}, 'arn': 'arn:aws:s3:::hacktoncloudapi-tokyojunctionvictimbucket-1dt0qaq4h9akn'}, 'object': {'key': 'raw/2.jpg', 'size': 1060, 'eTag': 'e38fd54eff55663d66d73ee82c2d5e6e', 'sequencer': '005C677009773A1139'}}}]}
    main(record, None)